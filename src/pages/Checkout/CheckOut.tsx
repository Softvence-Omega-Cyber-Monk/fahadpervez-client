...
const CheckoutPage = () => {
  ...
  const shippingMethods = {
    aramex: { name: 'Aramex', fee: 10, deliveryDays: 5, description: 'Standard delivery' },
    oreem: { name: 'Oreem', fee: 12, deliveryDays: 3, description: 'Express delivery' }
  };
  ...
  const selectedShipping = shippingMethods[formData.shippingMethod as keyof typeof shippingMethods];
  ...
  useEffect(() => {
    ...
    window.afsCompleteCallback = (resultIndicator: string) => {
      ...
      if (resultIndicator === session.successIndicator) {
        updateOrderAfterPayment(session.databaseOrderId!, session.orderId, resultIndicator);
      } else {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const orderData = {
        ...
        shippingMethodId: formData.shippingMethod === 'aramex' ? '6902ded28dc8ab84fb8481ee' : '69033edab5704c861a8cb4a2',
        totalPrice: subtotal,
      ...
      const databaseOrderId = orderResponse.data?.orderId || orderResponse._id || '';
  ...
  const updateOrderAfterPayment = async (databaseOrderId: string, transactionOrderId: string, resultIndicator: string) => {
    ...
    try {
      ...
      const updatePaymentResponse = await fetch(`http://localhost:5000/api/v1/orders/admin/${databaseOrderId}/payment-history`, {
        method: 'PUT',
        headers: { 
      ...
      if (updatePaymentData.success) {
        ...
        const successPageData = {
          ...
          orderResponse: {
            ...
            orderNumber: updatePaymentData.data?.orderNumber || databaseOrderId.slice(-8).toUpperCase(),
        ...
        throw new Error(updatePaymentData.error || 'Failed to update payment status');
      }
    } catch (error: any) {
              ...
              <div className="space-y-4">
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange}
                      className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 ${formErrors.fullName ? 'border-red-500' : 'border-gray-300'}`}
                  ...
                  <div>
                    ...
                    <input type="email" name="email" value={formData.email} onChange={handleInputChange}
                      className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 ${formErrors.email ? 'border-red-500' : 'border-gray-300'}`}
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="tel" name="phone" value={formData.phone} onChange={handleInputChange}
                      className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 ${formErrors.phone ? 'border-red-500' : 'border-gray-300'}`}
                ...
                <div>
                  ...
                  <input type="text" name="address" value={formData.address} onChange={handleInputChange}
                    className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 ${formErrors.address ? 'border-red-500' : 'border-gray-300'}`}
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input type="text" name="city" value={formData.city} onChange={handleInputChange}
                      className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 ${formErrors.city ? 'border-red-500' : 'border-gray-300'}`}
                  ...
                  <div>
                    ...
                    <input type="text" name="state" value={formData.state} onChange={handleInputChange}
                      className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 ${formErrors.state ? 'border-red-500' : 'border-gray-300'}`}
                      placeholder="NY" />
                  ...
                  <div>
                    ...
                    <input type="text" name="zipCode" value={formData.zipCode} onChange={handleInputChange}
                      className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 ${formErrors.zipCode ? 'border-red-500' : 'border-gray-300'}`}
                      placeholder="10001" />
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Order Remarks (Optional)</label>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            {method.description} - Delivery in {method.deliveryDays} days
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-gray-900">
                          {subtotal > 100 ? 'FREE' : `$${method.fee.toFixed(2)}`}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {cartItems.map((item) => (
                  <div key={item.id} className="flex gap-3 pb-3 border-b">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-semibold text-indigo-600">${item.totalPrice?.toFixed(2)}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between text-gray-600">
                  <div className="flex flex-col">
                    ...
                    {subtotal > 100 && <span className="text-xs text-green-600">Free shipping applied</span>}
                  </div>
              ...
              {paymentSuccess && (
                <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-green-700 mt-1">Your order has been confirmed. Redirecting...</p>
                  </div>
                </div>
              )}
