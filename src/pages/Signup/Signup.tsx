...
import LikeToSell from "@/components/AllRegister/LikeToSell/LikeToSell";

const Signup = () => {
  const [step, setStep] = useState<"basic" | "business" | "likeToSell" | "shipping" | "payment" | "contract" | "agreement" | "review" | "instraction" | "readsign">("basic");

  return (
    <div>
        {step === "basic" && <BasicInfo onNext={() => setStep("business")} />}
        {step === "business" && <BusinessInfo onPrevious={() => setStep("basic")} onNext={() => setStep("likeToSell")} />}
        {step === "likeToSell" && <LikeToSell onPrevious={() => setStep("likeToSell")} onNext={() => setStep("shipping")} />}
