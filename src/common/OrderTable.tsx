      ...
      <table className="w-full">
        <thead>
          <tr className="bg-gray-200 rounded-lg p-2">
            {["Order ID", "Seller", "Product", "Quantity", "Date", "Status", "Amount", "Action"].map((header) => (
              <th key={header} className="text-left py-3 px-4 font-medium text-gray-700 text-sm lg:text-md">
                {header}
              </th>
            ))}
          </tr>
          ...
          {currentOrders.map((order) => {
            ...
            return (
              <tr key={order._id} className="border-t border-gray-200 hover:bg-gray-50">
                ...
                <td className="py-4 px-4">
                  <span className={`inline-block px-3 py-1 rounded-full text-sm lg:text-md font-medium ${getStatusClasses(order.status)}`}>
                    {order.status}
                  </span>
                </td>
                  ...
                  <Link
                    to={
                      role === "ADMIN"
                        ...
                        : role === "VENDOR"
                        ? `/seller-dashboard/orders/order-detilse/${order._id}`
                        : `/buyer-dashboard/order-details/${order.orderNumber}`
                    }
                  >
                    <button className="text-sm lg:text-md text-blue-500 hover:text-blue-600 font-medium">View</button>
                  </Link>
                </td>
              </tr>
            );
        ...
        <thead>
          <tr className="bg-gray-200">
            {["Order ID", "Seller", "Product", "Date", "Status", "Amount", "Action"].map((header) => (
              <th key={header} className="text-left py-3 px-3 font-medium text-gray-700 text-xs md:text-sm">
                {header}
              </th>
            ))}
          </tr>
          ...
          {currentOrders.map((order) => (
            <tr key={order._id} className="border-t border-gray-200 hover:bg-gray-50">
              ...
              <td className="py-3 px-3">
                <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${getStatusClasses(order.status)}`}>
                  {order.status}
                </span>
              </td>
              ...
              <td className="py-3 px-3">
                <Link
                  to={
                    role === "ADMIN"
                      ? `/admin-dashboard/order-details/${order.orderNumber}`
                      : `/buyer-dashboard/order-details/${order.orderNumber}`
                  }
                >
                  <button className="text-sm lg:text-md text-blue-500 hover:text-blue-600 font-medium">View</button>
        ...
        return (
          <div key={order._id} className="border-t border-gray-200 p-4 hover:bg-gray-50">
            <div className="flex justify-between items-start mb-3">
              ...
              <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium ${getStatusClasses(order.status)}`}>
                {order.status}
              </span>
            </div>
...
//               >
//                 <td className="py-4 px-4 text-sm lg:text-md text-gray-900">
//                   {order.orderNumber}
//                 </td>
//                 <td className="py-4 px-4 text-sm lg:text-md text-gray-900">
//                   {order.userId.name}
...
//                   </span>
//                 </td>
//                 <td className="py-4 px-4 text-sm lg:text-md text-gray-900 font-medium">
//                   {order.grandTotal}
//                 </td>
//                 <td className="py-4 px-4">
//                   <Link
//                     to={role === "ADMIN" ? `/admin-dashboard/order-details/${order?._id}` : role === "VENDOR" ? `/seller-dashboard/orders/order-detilse/${order?._id}` : `/buyer-dashboard/order-details/${order?.orderNumber}` }
//                   >
...
//               className="border-t border-gray-200 hover:bg-gray-50"
//             >
//               <td className="py-3 px-3 text-xs md:text-sm text-gray-900">
//                 {order._id}
//               </td>
